...
type model =
  { entries : entry array (* Optimization:  Use a better data structure that actually exists in OCaml *)
  ; field : string
  ; uid : int
  ; visibility : string
  }
...
let viewEntries visibility entries =
  ...
  section
    ...
    ; ul
        [ class' "todo-list" ]
        (Array.map (fun todo -> if isVisible todo then lazy1 (string_of_int todo.id ^ string_of_bool todo.completed ^ string_of_bool todo.editing) (viewEntry todo) else noNode) entries |> Array.to_list)
]
